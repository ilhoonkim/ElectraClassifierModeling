...
class ElectraEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        #print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%", config.vocab_size, config.embedding_size, config.max_position_embeddings)
...
class ElectraBiLSTMClassificationHead(nn.Module):
    ...
    #     self.dense = nn.Linear(config.hidden_size, config.hidden_size)
    #     self.dropout = nn.Dropout(config.hidden_dropout_prob)
    #     self.lstm = nn.LSTM(input_size = config.hidden_size,  hidden_size = config.hidden_size, num_layers=1, bidirectional=True, batch_first=True)
    ...
    def forward(self, features, **kwargs):
        ...
        out, (hn, cn) = self.LSTM(x, (h_1,c_1))
        out = self.dropout(out)
        # print("아웃사이즈",out.size())
        final_state = out[:,-1,:]
...
class ElectraClassificationHead(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        # self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.dense = nn.Linear(config.hidden_size *32 , 768)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.out_proj = nn.Linear(config.hidden_size, config.num_labels)
    ...
    def forward(self, features, **kwargs):
        ...
        x = features.view(batch_size,1, -1)
        x = self.dropout(x)
        x = self.dense(x)
...
class ElectraForSequenceClassification(ElectraPreTrainedModel):
    def __init__(self, config):
        ...
        # self.classifier = ElectraCnnClassificationHead(config)
        # self.classifier = ElectraBiLSTMClassificationHead(config)
        self.init_weights()
    ...
    def forward(
        ...
        if labels is not None:
            ...
            # elif self.num_labels == 2: #### 바이너리 클래스인 경우에 로스 함수 변경
            #     loss_fct = BCEWithLogitsLoss()
            #     loss = loss_fct(logits, labels.unsqueeze(1))
            else:
                loss_fct = CrossEntropyLoss()
                print("type_label",type(labels))
...
#         self.layer1 = torch.nn.Sequential(
#             torch.nn.Conv2d(in_channels=1 ,out_channels=64, kernel_size=(1,256), stride=1),
#             torch.nn.ReLU(),
...
class CNN(torch.nn.Module):
    def __init__(self):
        super(CNN, self).__init__()
        self.layer1 = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=1 ,out_channels=512, kernel_size=(1,768), stride=1),
            torch.nn.ReLU(),
        )
        self.layer2 = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=1 ,out_channels=256, kernel_size=(1,512), stride=1),
            torch.nn.ReLU(),
        )
        self.layer3 = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=1 ,out_channels=256, kernel_size=(1,256), stride=1),
...
#         conv1_layer = torch.nn.Conv2d(in_channels=1 ,out_channels=4096, kernel_size=(3,256), stride=1).cuda()
#         conv1 = conv1_layer(input_tensor)
#         pool1_layer = torch.nn.MaxPool2d(1,510).cuda()
#         pool1 = pool1_layer(conv1)
#         output_tensor = torch.reshape(pool1,(batch_size,512,8))
